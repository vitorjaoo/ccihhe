...
const App = {
  ...
  async salvarPaciente() {
    ...
    // Bloqueia por 5s após sucesso também (evita duplo clique acidental)
    setTimeout(() => {
      State.salvarPacienteLocked = false;
      if (btn) { btn.disabled = false; btn.textContent = 'Internar'; }
    }, 5000);
  ...
  async deletarPaciente() {
    if (State.user.nivel_acesso !== 'admin') return toast('Apenas administradores podem excluir pacientes.', 'error');
    const p = State.currentPacienteData;
    if (!p) return;
    const confirma = confirm(`⚠️ ATENÇÃO: Isso irá excluir permanentemente o paciente "${p.nome}" e todos os seus registros, procedimentos e infecções.\n\nEsta ação NÃO pode ser desfeita. Confirmar?`);
    if (!confirma) return;
    try {
  ...
  renderPacienteDetail() {
    ...
    if (dxEl) {
      dxEl.textContent = p.diagnostico ? `Dx: ${p.diagnostico}` : '';
      dxEl.style.display = p.diagnostico ? 'block' : 'none';
    }
    ...
    if (diasEl) {
      diasEl.innerHTML = diasInternacao !== null
        ? `<span class="dias-badge ${diasInternacao >= 14 ? 'dias-danger' : diasInternacao >= 7 ? 'dias-warn' : 'dias-ok'}">🏥 ${diasInternacao} dia(s) internado</span>`
        : '';
    }
    ...
    if (transfBanner) {
      if (p.status === 'transito' || p.setor_destino_id != null) {
        ...
        transfBanner.style.display = 'none';
      }
    }
