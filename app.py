    ...
    def fetchone(self):
        ...
        class RowDict(dict):
            ...
            def __getitem__(self, key):
                if isinstance(key, int): return self.t[key]
...
class TursoAdapter:
   =def __init__(self):
    ...
    def executescript(self, sql_script):
        ...
        if statements: self.client.batch(statements)

    def commit(self): pass
    def close(self): self.client.close()

def get_db(): return TursoAdapter()
...
def init_db():
    try:
        ...
        for m in motivos: c.execute("INSERT OR IGNORE INTO motivos_saida(nome) VALUES(?)", (m,))
        ...
        if not admin_check:
            c.execute("INSERT INTO usuarios(nome, email, senha, nivel_acesso, setor_id) VALUES('Administrador', 'admin@ccih.com', ?, 'admin', NULL)", (hash_password('admin123'),))
        conn.commit()
        conn.close()
...
def not_readonly(f):
    @wraps(f)
    def decorated(*args, **kwargs):
        if session.get('nivel_acesso') == 'espectador': return jsonify({'error': 'Acesso somente leitura'}), 403
        return f(*args, **kwargs)
    return decorated
...
def login():
    ...
    email, senha = data.get('email', '').strip().lower(), data.get('senha', '')
    if not email or not senha: return jsonify({'error': 'Campos obrigatórios'}), 400
    conn = get_db()
    user = conn.execute("SELECT * FROM usuarios WHERE email=?", (email,)).fetchone()
    conn.close()
    if not user or user['senha'] != hash_password(senha): return jsonify({'error': 'Credenciais inválidas'}), 401
    session['user_id'], session['nivel_acesso'], session['setor_id'], session['nome'] = user['id'], user['nivel_acesso'], user['setor_id'], user['nome']
...
def change_password():
    data = request.json or {}
    senha_atual, nova_senha = data.get('senha_atual', ''), data.get('nova_senha', '')
    conn = get_db()
...
@not_readonly
def create_setor():
    if session['nivel_acesso'] != 'admin': return jsonify({'error': 'Apenas admin'}), 403
...
@login_required
def get_usuarios():
    if session['nivel_acesso'] not in ('admin', 'espectador'): return jsonify({'error': 'Sem permissão'}), 403
    conn = get_db()
    rows = conn.execute("SELECT u.id, u.nome, u.email, u.nivel_acesso, u.setor_id, s.nome as setor_nome FROM usuarios u LEFT JOIN setores s ON s.id=u.setor_id ORDER BY u.nome").fetchall()
    conn.close()
    return jsonify(rows)
...
def create_usuario():
    if session['nivel_acesso'] != 'admin': return jsonify({'error': 'Apenas admin'}), 403
    data = request.json or {}
    nome, email, senha, nivel, setor_id = data.get('nome','').strip(), data.get('email','').strip().lower(), data.get('senha','').strip(), data.get('nivel_acesso',''), data.get('setor_id') or None
    if not all([nome, email, senha, nivel]) or nivel not in ('admin', 'estagiario', 'espectador'): return jsonify({'error': 'Dados inválidos'}), 400
    conn = get_db()
    try:
        conn.execute("INSERT INTO usuarios(nome,email,senha,nivel_acesso,setor_id) VALUES(?,?,?,?,?)", (nome, email, hash_password(senha), nivel, setor_id))
        conn.commit()
...
@login_required
def get_pacientes():
    nivel, setor_id = session['nivel_acesso'], session['setor_id']
    conn = get_db()
    try:
        if nivel == 'estagiario' and setor_id:
            rows = conn.execute("SELECT p.*, s.nome as setor_nome FROM pacientes p LEFT JOIN setores s ON s.id = p.setor_id_atual WHERE p.setor_id_atual=? AND p.status='internado' ORDER BY p.nome", (setor_id,)).fetchall()
        else:
            rows = conn.execute("SELECT p.*, s.nome as setor_nome FROM pacientes p LEFT JOIN setores s ON s.id = p.setor_id_atual WHERE p.status='internado' ORDER BY s.nome, p.nome").fetchall()
        for p in rows:
            p['procedimentos'] = conn.execute("SELECT * FROM procedimentos WHERE paciente_id=? AND status='ativo'", (p['id'],)).fetchall()
            p['infeccoes'] = conn.execute("SELECT * FROM infeccoes_notificadas WHERE paciente_id=?", (p['id'],)).fetchall()
    except Exception as e:
        conn.close()
...
@not_readonly
def create_paciente():
    if session['nivel_acesso'] == 'espectador': return jsonify({'error': 'Sem permissão'}), 403
    data = request.json or {}
    nome = data.get('nome', '').strip()
    if not nome: return jsonify({'error': 'Nome obrigatório'}), 400
    setor_id, idade = data.get('setor_id'), data.get('idade')
    if not setor_id or str(setor_id).strip() == '': setor_id = None
    if not idade or str(idade).strip() == '': idade = None
    if session['nivel_acesso'] == 'estagiario': setor_id = session['setor_id']

    conn = get_db()
    try:
        cursor = conn.execute("INSERT INTO pacientes(nome, idade, leito, prontuario, fone, setor_id_atual, status) VALUES(?,?,?,?,?,?,'internado')", (nome, idade, data.get('leito', '').strip(), data.get('prontuario', '').strip(), data.get('fone', '').strip(), setor_id))
        conn.commit()
        pac = conn.execute("SELECT p.*, s.nome as setor_nome FROM pacientes p LEFT JOIN setores s ON s.id=p.setor_id_atual WHERE p.id=?", (cursor.lastrowid,)).fetchone()
    except Exception as e:
        conn.close()
        return jsonify({'error': str(e)}), 500
    conn.close()
...
def get_paciente(pid):
    ...
    pac['registros'] = conn.execute("SELECT * FROM registros_diarios WHERE paciente_id=? ORDER BY data DESC", (pid,)).fetchall()
    pac['procedimentos'] = conn.execute("SELECT * FROM procedimentos WHERE paciente_id=? ORDER BY data_insercao DESC", (pid,)).fetchall()
    pac['infeccoes'] = conn.execute("SELECT * FROM infeccoes_notificadas WHERE paciente_id=? ORDER BY data_notificacao DESC", (pid,)).fetchall()
    conn.close()
    return jsonify(pac)
...
def dar_alta(pid):
    ...
    if not motivo_id: return jsonify({'error': 'Motivo obrigatório'}), 400
    conn = get_db()
...
def add_registro(pid):
    data = request.json or {}
    conn = get_db()
    conn.execute("INSERT INTO registros_diarios(paciente_id, data, temperatura) VALUES(?,?,?)", (pid, data.get('data') or date.today().isoformat(), data.get('temperatura')))
    conn.commit()
    reg = conn.execute("SELECT * FROM registros_diarios WHERE paciente_id=? ORDER BY id DESC LIMIT 1", (pid,)).fetchone()
    conn.close()
...
def add_procedimento(pid):
    data = request.json or {}
    tipo, data_ins = data.get('tipo_procedimento', '').strip(), data.get('data_insercao', '').strip()
    if not tipo or not data_ins: return jsonify({'error': 'Dados obrigatórios'}), 400
    conn = get_db()
    conn.execute("INSERT INTO procedimentos(paciente_id, tipo_procedimento, data_insercao, status) VALUES(?,?,?,'ativo')", (pid, tipo, data_ins))
    conn.commit()
    proc = conn.execute("SELECT * FROM procedimentos WHERE paciente_id=? ORDER BY id DESC LIMIT 1", (pid,)).fetchone()
    conn.close()
...
def add_infeccao(pid):
    ...
    if tipo not in TIPOS_INFECCAO: return jsonify({'error': 'Tipo inválido'}), 400
    conn = get_db()
    conn.execute("INSERT INTO infeccoes_notificadas(paciente_id, tipo_infeccao, data_notificacao) VALUES(?,?,?)", (pid, tipo, data.get('data_notificacao') or date.today().isoformat()))
    conn.commit()
    inf = conn.execute("SELECT * FROM infeccoes_notificadas WHERE paciente_id=? ORDER BY id DESC LIMIT 1", (pid,)).fetchone()
    conn.close()
...
# ---------------------------------------------------------------------------
# DASHBOARD REESCRITO PARA SUPORTAR SETOR DO USUÁRIO
# ---------------------------------------------------------------------------
...
def relatorios():
    ...
    conn = get_db()
    try:
        pac_alta_row = conn.execute(f"SELECT COUNT(DISTINCT p.id) as total FROM pacientes p WHERE p.status='alta' {wp}", params).fetchone()
        pacientes_alta = int(pac_alta_row['total']) if pac_alta_row else 0

        inf_mes_row = conn.execute(f"SELECT COUNT(i.id) as total FROM infeccoes_notificadas i JOIN pacientes p ON p.id = i.paciente_id WHERE strftime('%Y-%m', i.data_notificacao)=? {wp}", [ano_mes] + params).fetchone()
        total_inf = int(inf_mes_row['total']) if inf_mes_row else 0

        por_tipo = conn.execute(f"SELECT i.tipo_infeccao, COUNT(i.id) as total FROM infeccoes_notificadas i JOIN pacientes p ON p.id = i.paciente_id WHERE strftime('%Y-%m', i.data_notificacao)=? {wp} GROUP BY i.tipo_infeccao", [ano_mes] + params).fetchall()
        ...
        tot_cat = conn.execute(f"SELECT COUNT(DISTINCT pr.paciente_id) as total FROM procedimentos pr JOIN pacientes p ON p.id = pr.paciente_id WHERE lower(pr.tipo_procedimento) IN (?, ?, ?) {wp}", cateteres + tuple(params)).fetchone()
        tot_cateter = int(tot_cat['total']) if tot_cat else 0

        sepse_cat = conn.execute(f"SELECT COUNT(DISTINCT i.paciente_id) as total FROM infeccoes_notificadas i JOIN procedimentos pr ON i.paciente_id = pr.paciente_id JOIN pacientes p ON p.id = i.paciente_id WHERE i.tipo_infeccao = 'Sepse' AND lower(pr.tipo_procedimento) IN (?, ?, ?) AND strftime('%Y-%m', i.data_notificacao) = ? {wp}", cateteres + (ano_mes,) + tuple(params)).fetchone()
        ...
        tot_resp = conn.execute(f"SELECT COUNT(DISTINCT pr.paciente_id) as total FROM procedimentos pr JOIN pacientes p ON p.id = pr.paciente_id WHERE lower(pr.tipo_procedimento) IN (?, ?) {wp}", resps + tuple(params)).fetchone()
        tot_resp_int = int(tot_resp['total']) if tot_resp else 0

        pneu_resp = conn.execute(f"SELECT COUNT(DISTINCT i.paciente_id) as total FROM infeccoes_notificadas i JOIN procedimentos pr ON i.paciente_id = pr.paciente_id JOIN pacientes p ON p.id = i.paciente_id WHERE i.tipo_infeccao = 'Pneumonia' AND lower(pr.tipo_procedimento) IN (?, ?) AND strftime('%Y-%m', i.data_notificacao) = ? {wp}", resps + (ano_mes,) + tuple(params)).fetchone()
        ...
        tot_sonda = conn.execute(f"SELECT COUNT(DISTINCT pr.paciente_id) as total FROM procedimentos pr JOIN pacientes p ON p.id = pr.paciente_id WHERE lower(pr.tipo_procedimento) = 'sonda vesical' {wp}", tuple(params)).fetchone()
        tot_sonda_int = int(tot_sonda['total']) if tot_sonda else 0

        uri_sonda = conn.execute(f"SELECT COUNT(DISTINCT i.paciente_id) as total FROM infeccoes_notificadas i JOIN procedimentos pr ON i.paciente_id = pr.paciente_id JOIN pacientes p ON p.id = i.paciente_id WHERE i.tipo_infeccao = 'Trato Urinário' AND lower(pr.tipo_procedimento) = 'sonda vesical' AND strftime('%Y-%m', i.data_notificacao) = ? {wp}", (ano_mes,) + tuple(params)).fetchone()
        ...
        por_setor = conn.execute(f"SELECT s.nome as setor, COUNT(p.id) as total FROM pacientes p JOIN setores s ON s.id=p.setor_id_atual WHERE p.status='internado' {wp} GROUP BY s.id ORDER BY s.nome", tuple(params)).fetchall()
        ultimas_inf = conn.execute(f"SELECT i.*, p.nome as paciente_nome, s.nome as setor_nome FROM infeccoes_notificadas i JOIN pacientes p ON p.id=i.paciente_id LEFT JOIN setores s ON s.id=p.setor_id_atual WHERE 1=1 {wp} ORDER BY i.id DESC LIMIT 10", tuple(params)).fetchall()
...
# ---------------------------------------------------------------------------
# RELATÓRIO IMPRESSO PARA O ADMIN (COM AVISO E LOGO)
# ---------------------------------------------------------------------------
...
def relatorio_impresso():
    ...
    <head>
        ...
        <style>
            ...
            .btn-print {{ background: #0d9488; color: white; padding: 10px 20px; text-decoration: none; border-radius: 6px; font-weight: bold; cursor: pointer; border: none; }}
